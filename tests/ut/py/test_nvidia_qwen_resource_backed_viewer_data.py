...
def test_viewer_results_include_resource_backed_diagnostic_rows():
    ...
    first_layer_logits_4step_rows = [
        row
        for row in rows
        if row["raw_artifact"].startswith(
            "tmp/cuda-backend/qwen-first-layer-logits-4step/",
        )
    ]
    first_layer_logits_16step_rows = [
        row
        for row in rows
        if row["raw_artifact"].startswith(
    ...
    mpk_64step_logits_rows = [
        row
        for row in rows
        if row["raw_artifact"].startswith(
            "tmp/cuda-backend/qwen-first-layer-logits-mpk-64step-final/",
        )
    ]
    ...
    assert len(mpk_64step_logits_rows) == 1
    assert all(
    ...
    assert all(
        ...
        and row["statistic"].get("task_func_id_sequence") == list(range(7100, 7110))
        and row["statistic"].get("last_completed_count") == 10
        and row["statistic"].get("executed_decode_steps") == 4
        and row["statistic"].get("logits_check_policy") == "every_step"
        and row["statistic"].get("logits_checked_step_count") == 4
        and row["statistic"].get("decode_feedback_applied_step_count") == 4
        and row["statistic"].get("decode_feedback_status")
        == "device_token_feedback_observed"
        and row["statistic"].get("diagnostic_logits_reference_status") == "pass"
        and row["correctness"] == "pass"
        for row in first_layer_logits_4step_rows
    )
    assert all(
        row["statistic"].get("task_selection") == "first_layer_with_logits"
        and row["statistic"].get("task_coverage_count") == 10
        and row["statistic"].get("task_func_id_sequence") == list(range(7100, 7110))
        and row["statistic"].get("last_completed_count") == 10
        and row["statistic"].get("executed_decode_steps") == 16
        and row["statistic"].get("logits_check_policy") == "every_step"
        and row["statistic"].get("logits_checked_step_count") == 16
        and row["statistic"].get("decode_feedback_applied_step_count") == 16
        and row["statistic"].get("decode_feedback_status")
        == "device_token_feedback_observed"
        and row["statistic"].get("diagnostic_logits_reference_status") == "pass"
        and row["correctness"] == "pass"
    ...
    assert all(
        row["statistic"].get("workload_id") == "mpk_offline_decode"
        and row["statistic"].get("task_selection") == "first_layer_with_logits"
        and row["statistic"].get("task_coverage_count") == 10
        and row["statistic"].get("task_func_id_sequence") == list(range(7100, 7110))
        and row["statistic"].get("last_completed_count") == 10
        and row["statistic"].get("planned_decode_steps") == 1024
        and row["statistic"].get("executed_decode_steps") == 64
        and row["statistic"].get("logits_check_policy") == "final_step"
        and row["statistic"].get("logits_checked_step_count") == 1
        and row["statistic"].get("logits_deferred_step_count") == 63
        ...
        and row["statistic"].get("diagnostic_logits_reference_status") == "pass"
        and row["correctness"] == "pass"
        for row in mpk_64step_logits_rows
    )
    assert all(
