...
def test_render_persistent_dag_source_records_device_scheduler_errors():
    ...
    assert "pto_dag_push_ready(state, idx);" in source
    assert "atomicAdd(state->scheduler_init_count, 1U);" in source
    assert "while (atomicAdd(state->scheduler_init_count, 0U) < scheduler_blocks" in source
    assert "state->task_count != 0ULL && atomicAdd(state->queue_tail, 0U) == 0U" in source
