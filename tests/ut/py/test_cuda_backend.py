...
def test_cuda_pto_graph_recorder_captures_python_orchestration_submits():
    from simpler.task_interface import CallConfig, ContinuousTensor, DataType, TaskArgs, TensorArgType
    from simpler_setup.cuda_pto_graph import record_cuda_pto_submits, lower_cuda_pto_task_graph

    def tensor(ptr):
        return ContinuousTensor.make(ptr, (4,), DataType.FLOAT32, False)

    def orch(orch_ctx, _args, cfg):
        root_args = TaskArgs()
        root_args.add_tensor(tensor(10), TensorArgType.INPUT)
        root_args.add_tensor(tensor(30), TensorArgType.OUTPUT)
        middle_args = TaskArgs()
        middle_args.add_tensor(tensor(30), TensorArgType.INPUT)
        middle_args.add_tensor(tensor(40), TensorArgType.OUTPUT)
        orch_ctx.submit_next_level(101, root_args, cfg)
        orch_ctx.submit_next_level(102, middle_args, cfg)

    submits = record_cuda_pto_submits(orch, None, CallConfig(), tensor_names=(("a", "tmp0"), ("tmp0", "tmp1")))
    lowered = lower_cuda_pto_task_graph(
        submits,
        lambda node, begin, count, fanin: (node.key, node.func_id, begin, count, fanin),
    )

    assert [submit.callable_id for submit in submits] == [101, 102]
    assert lowered.fanin == [0, 1]
    assert lowered.dependents == [1]
...
assert result["runtime"] == "persistent_device"
assert result["mode"] == "dag"
assert result["dag_shape"] == "graph_descriptor_multi_fanin"
...
assert result["scheduler_init_count"] == 2
assert result["scheduler_loop_count"] == 2
assert result["scheduler_processed_count"] == 4
assert len(result["scheduler_processed_by_block"]) == 2
assert sum(result["scheduler_processed_by_block"]) == 4
assert result["dispatch_func_ids"] == [1, 2, 11, 6]
...
try:
    ...
    raise AssertionError("expected persistent DAG self-dependent scheduler error")
"""
    result = subprocess.run(
        [sys.executable, "-c", script],
        check=False,
...
assert result["runtime"] == "persistent_device"
assert result["mode"] == "dag"
assert result["dag_shape"] == "graph_descriptor_triad"
assert result["task_count"] == 3
assert result["queue_capacity"] == 2
assert result["completed_count"] == 3
assert result["dispatch_func_ids"] == [6, 2, 1]
...
assert result["runtime"] == "persistent_device"
assert result["mode"] == "dag"
assert result["dag_shape"] == "graph_descriptor_quad"
assert result["task_count"] == 3
assert result["queue_capacity"] == 2
assert result["completed_count"] == 3
assert result["dispatch_func_ids"] == [8, 2, 1]
...
result = run_persistent_smoke(
    ...
    mode="dag",
    queue_capacity=2,
    dag_shape="graph_descriptor_node_port_dict",
)
assert result["status"] == "pass"
assert result["runtime"] == "persistent_device"
assert result["mode"] == "dag"
assert result["dag_shape"] == "graph_descriptor_node_port_dict"
assert result["task_count"] == 3
assert result["queue_capacity"] == 2
assert result["completed_count"] == 3
assert result["dispatch_func_ids"] == [1, 2, 1]
...
assert result["source_kind"] == "generated-dispatch"
assert result["graph_node_ops"] == {
    "task0": "op:add=1",
    "task1": "op:mul=2",
    "task2": "op:add=1",
}
assert result["graph_task_arg_key"] == "node_port_dict"
...
assert result["runtime"] == "persistent_device"
assert result["mode"] == "dag"
assert result["dag_shape"] == "graph_descriptor_named_callable"
assert result["task_count"] == 3
assert result["queue_capacity"] == 2
assert result["completed_count"] == 3
assert result["dispatch_func_ids"] == [1, 2, 1]
...
result = run_persistent_smoke(
    ...
    mode="dag",
    queue_capacity=2,
    dag_shape="graph_descriptor_submits",
)
assert result["status"] == "pass"
assert result["runtime"] == "persistent_device"
assert result["mode"] == "dag"
assert result["dag_shape"] == "graph_descriptor_submits"
assert result["task_count"] == 3
assert result["queue_capacity"] == 2
assert result["completed_count"] == 3
assert result["dispatch_func_ids"] == [1, 1, 1]
assert result["fanin_remaining"] == [0, 0, 0]
assert result["device_scheduler_errors"] == {"count": 0, "code": 0, "task_id": 0}
assert result["graph_descriptor"] == {
    "tasks": 3,
...
assert result["runtime"] == "persistent_device"
assert result["mode"] == "dag"
assert result["dag_shape"] == "graph_descriptor_submit_groups"
assert result["task_count"] == 3
assert result["queue_capacity"] == 2
assert result["completed_count"] == 3
assert result["dispatch_func_ids"] == [1, 1, 1]
