...
def test_runtime_dispatch_driver_status_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "missing driver remains handoff-owned failed",
        "stale accepted driver is driver-owned failed",
        "`driver_missing`",
        "`driver_stale`",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-25 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Status Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        "multi-agent worker id `019eff17-8630-79c3-bbde-f995c903fe9a`",
        "nickname `Lovelace`",
        "No tmux pane is used for this worker",
        "No nested workers were launched",
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `bb8e6730414794002be80b19b7191feb415dfbb7`",
        "PR #185 <https://github.com/uv-xiao/pto-cu/pull/185>",
        "opened as a non-draft PR",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "nvidia-uccl-ep-runtime-fusion-runtime-dispatch-driver-status-map",
        "docs/in_progress/nvidia_backend/dispatch_log.md",
        ...
        "Accepted only for the private driver-owned unsupported/failed status",
        "no real UCCL-EP dispatch/combine work",
        "no scheduler/runtime pass evidence",
        "no fresh H200 fused success",
        "does not report `persistent_device_uccl_ep_runtime_fusion.status: passed`",
        ...
        assert required in normalized_dispatch
    assert "pending PR creation" not in normalized_dispatch
    assert "/home/" not in normalized_dispatch


def test_runtime_dispatch_driver_scaffold_status_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "valid prepared driver scaffold",
        "PR #183 handoff",
        "same invocation id",
        "runtime-owned output sink",
        "driver_unsupported_boundary",
    ...
    for required in [
        ...
        "pto_cuda_runtime_fusion_prepare_runtime_dispatch_driver_scaffold_status",
        "pto_cuda_runtime_fusion_failure_is_runtime_dispatch_driver_failed",
        "private UCCL-EP runtime dispatch driver scaffold/status validation failed",
        "driver_owner_mismatch",
        "driver_invocation_mismatch",
        "driver_runtime_path_mismatch",
        "driver_descriptor_token_mismatch",
    ...
    for required in [
        "test_private_runtime_dispatch_driver_scaffold_status_is_driver_owned",
        "PTO_CUDA_RUNTIME_FUSION_FAILURE_DRIVER_OWNER_MISMATCH",
        "PTO_CUDA_RUNTIME_FUSION_FAILURE_DRIVER_INVOCATION_MISMATCH",
        "pto_cuda_uccl_ep_runtime_dispatch_driver_status_name",
        "private UCCL-EP runtime dispatch driver scaffold/status",
    ]:
        assert required in private_entry_test

    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-25 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Scaffold Status Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        "multi-agent worker id `019eff26-10a1-7872-bf4f-3f533d1a9db7`",
        "nickname `Peirce`",
        "No tmux pane is used for this worker",
        "No nested workers were launched",
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `8619767d0eacb5c870b6a56337c6bcb380a2af75`",
        "PR #186 <https://github.com/uv-xiao/pto-cu/pull/186>",
        "opened as a non-draft PR",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "test_private_runtime_dispatch_driver_scaffold_status_is_driver_owned",
        "1 failed in 0.40s",
...
def test_runtime_dispatch_driver_backend_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "future private runtime dispatch driver's request/backend ownership",
        "after PR #186",
        "7589e2df44ad4df9c200cd4ec673dacac0a27a71",
        "private driver request owner",
        "dispatch backend placeholder",
        "combine backend placeholder",
        "status sink owner",
        "driver-owned failure propagation",
        "PtoCudaUcclEpRuntimeDispatchDriverScaffoldStatus",
        "PtoCudaUcclEpRuntimeDispatchDriverStatus",
        "valid prepared driver scaffold remains `unsupported`",
        ...
        "invalid pass-evidence boundary",
        "example JSON",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-25 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        "multi-agent worker id `019eff44-797d-7fd2-9138-b113d268e2c8`",
        "nickname `Helmholtz`",
        "No tmux pane is used for this worker",
        "No nested workers were launched",
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `9e338948f90fdc4fb13a527159060b2510e12838`",
        "planned PR slot #188",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "nvidia-uccl-ep-runtime-fusion-runtime-dispatch-driver-backend-map",
        "docs/in_progress/nvidia_backend/dispatch_log.md",
...
def test_runtime_dispatch_driver_backend_request_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "future private driver backend request",
        "after PR #189",
        "707cc81a818fdc00e4f592acb2f17538d1f6eb0a",
        "private backend request owner",
        "backend scaffold/status input",
        "dispatch request placeholder",
        "combine request placeholder",
        "descriptor token validation",
        "rank/device validation",
        "status sink ownership",
        ...
        "selected exactly one next PR-sized implementation slice",
        "private backend request scaffold/status",
        "no real UCCL-EP dispatch/combine work",
        "no scheduler/runtime pass evidence",
        "no fresh H200 fused success",
        "no examples, stable docs, or performance claims",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-25 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Request Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        "multi-agent worker id `019eff67-0076-7c31-86a6-9e7d5516c265`",
        "nickname `Hilbert`",
        "No tmux pane is used for this worker",
        "No nested workers were launched",
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `707cc81a818fdc00e4f592acb2f17538d1f6eb0a`",
        "planned PR slot #190",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "nvidia-uccl-ep-runtime-fusion-runtime-dispatch-driver-backend-request-map",
        "docs/in_progress/nvidia_backend/dispatch_log.md",
...
def test_runtime_dispatch_driver_backend_request_scaffold_status_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-25 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Request Scaffold Status Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        "multi-agent worker id `019eff72-bb96-77a1-9b14-2601dd6b3f11`",
        "nickname `Bohr`",
        "No tmux pane is used for this worker",
        ...
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `4223edd9fa3c5e58b62eff1d7c27b1a54670766d`",
        "planned PR slot #191",
        "actual PR #191",
        "https://github.com/uv-xiao/pto-cu/pull/191",
        "non-draft PR",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "src/cuda/platform/include/host/pto_cuda_runtime_fusion_abi.h",
        "tests/ut/py/test_cuda_runtime_fusion_private_entry.py",
        ...
        "focused red check failed first with `1 failed in 0.40s`",
        "focused green check passed with `1 passed in 0.42s`",
        "final focused private-entry recheck passed with `1 passed in 0.41s`",
        "full private-entry pytest passed with `19 passed in 5.11s`",
        "`git diff --check` passed with no output",
        "`Summary: 0 error(s)`",
        "`nvidia review guard passed`",
...
def test_runtime_dispatch_driver_backend_dispatch_request_scaffold_status_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "`driver_backend_dispatch_request_fabricated_pass_evidence`",
        "focused red check failed first",
        "focused green check passed with `1 passed in 0.41s`",
        "no real UCCL-EP dispatch/combine work",
        "no scheduler/runtime pass evidence",
        "no fresh H200 fused success",
        "no public `TaskArgs`",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Dispatch Request Scaffold Status Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        ...
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `14aaedd8865ea7351cd30ee1a0dc46804b7d0f36`",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "src/cuda/platform/include/host/pto_cuda_runtime_fusion_abi.h",
        "tests/ut/py/test_cuda_runtime_fusion_private_entry.py",
        "tests/ut/py/test_nvidia_review_artifacts.py",
        ...
        "docs/in_progress/nvidia_backend/communication_selection.md",
        "docs/in_progress/nvidia_backend/persistent_moe_dispatch_combine_h200.md",
        "focused red check failed first with `1 failed in 0.41s`",
        "focused green check passed with `1 passed in 0.41s`",
        "`git diff --check` passed with no output",
        "`Summary: 0 error(s)`",
        "`nvidia review guard passed`",
...
def test_runtime_dispatch_driver_backend_combine_request_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "future private combine request placeholder",
        "after PR #193",
        ...
        "private combine request placeholder owner",
        "combine payload descriptor placeholder",
        "combine output/status sink",
        "descriptor token validation",
        "rank/device validation",
        "invalid public/provenance sources",
        ...
        "map unsupported boundary",
        "payload transfer unimplemented",
        "public `TaskArgs`",
        "public `CallConfig`",
        "common runtime C API",
        "UCCL host-runtime ABI",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Request Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        "multi-agent worker id `019effb1-2367-7122-bf01-818bf91ed31d`",
        "nickname `Faraday`",
        "No tmux pane is used for this worker",
        "the dispatcher monitors through multi-agent wait",
        "No nested workers were launched",
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `f969ea00c6858a6633ee53fd33bf77dd434097dc`",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "nvidia-uccl-ep-runtime-fusion-runtime-dispatch-driver-backend-combine-request-map",
        "docs/in_progress/nvidia_backend/dispatch_log.md",
        "docs/in_progress/nvidia_backend/pr_slicing_plan.md",
...
def test_runtime_dispatch_driver_backend_combine_payload_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "future private combine payload descriptor placeholder",
        "after PR #195",
        ...
        "private combine payload descriptor placeholder owner",
        "combine payload output/status sink",
        "descriptor token validation",
        "rank/device validation",
        "invalid public/provenance sources",
        ...
        "selected exactly one next PR-sized implementation slice",
        "private combine payload descriptor scaffold/status only",
        "no real UCCL-EP dispatch/combine work",
        "no scheduler/runtime pass evidence",
        "no fresh H200 fused success",
        "no examples, stable docs, serving, vLLM, DeepSeek, or performance claims",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Payload Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        "multi-agent worker id `019effd6-2dfb-77f3-9929-279178b85ea0`",
        "nickname `McClintock`",
        "No tmux pane is used for this worker",
        "the dispatcher monitors through multi-agent wait",
        "No nested workers were launched",
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `e09b67a7a00f481f8c9dd4508d1adc9e88030d00`",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "nvidia-uccl-ep-runtime-fusion-runtime-dispatch-driver-backend-combine-payload-map",
        "docs/in_progress/nvidia_backend/dispatch_log.md",
        "docs/in_progress/nvidia_backend/pr_slicing_plan.md",
...
def test_runtime_dispatch_driver_backend_combine_payload_scaffold_status_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "private combine payload descriptor scaffold/status",
        "PR #196",
        ...
        "valid prepared combine payload descriptor scaffold/status remains `unsupported`",
        "`actual_fused_cross_gpu_execution` remains `0`",
        "`driver_backend_combine_payload_pending`",
        "`driver_backend_combine_payload_descriptor_placeholder`",
        "`driver_backend_combine_payload_output_status_sink_unbound`",
        ...
        "`driver_backend_combine_payload_provenance_sourced_state`",
        "`driver_backend_combine_payload_fabricated_pass_evidence`",
        "focused red check failed first with `1 failed in 0.44s`",
        "focused green check passed with `1 passed in 0.43s`",
        "no real UCCL-EP dispatch/combine work",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Payload Scaffold Status Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        "multi-agent worker id `019effe5-7feb-7ed0-8830-728123b163dc`",
        "nickname `Pascal`",
        "No tmux pane is used for this worker",
        "the dispatcher monitors through multi-agent wait",
        "No nested workers were launched",
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `af62d14d456b34fb1ef7fb2f9b4b6af7bc0bd4d1`",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "src/cuda/platform/include/host/pto_cuda_runtime_fusion_abi.h",
        "tests/ut/py/test_cuda_runtime_fusion_private_entry.py",
        "tests/ut/py/test_nvidia_review_artifacts.py",
...
def test_runtime_dispatch_driver_backend_combine_payload_transfer_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "future private combine payload transfer boundary",
        "dependency map only",
        "must not implement source behavior",
        "PR #197",
        ...
        "private combine payload transfer boundary owner",
        "combine payload transfer output/status sink",
        "descriptor token validation",
        "rank/device validation",
        "invalid public/provenance sources",
        ...
        "selected exactly one next PR-sized implementation slice",
        "private combine payload transfer scaffold/status only",
        "no real UCCL-EP dispatch/combine work",
        "no descriptor allocation behavior",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Payload Transfer Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        ...
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `3337516c95fcd5f6129c515585d92e3f95f0c444`",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "nvidia-uccl-ep-runtime-fusion-runtime-dispatch-driver-backend-combine-payload-transfer-map",
        "docs/in_progress/nvidia_backend/dispatch_log.md",
        "docs/in_progress/nvidia_backend/pr_slicing_plan.md",
...
def test_runtime_dispatch_driver_backend_combine_payload_transfer_completion_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "selected exactly one next PR-sized implementation slice",
        "private completion scaffold/status only",
    ]
    for name, text in texts.items():
        normalized = " ".join(text.split())
        for required in required_terms:
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Payload Transfer Completion Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        ...
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `41c9c894ad511534d943180bccb10aab8fba3f7b`",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "docs/in_progress/nvidia_backend/dispatch_log.md",
        "docs/in_progress/nvidia_backend/pr_slicing_plan.md",
        "docs/in_progress/nvidia_backend/communication_runtime_boundary.md",
...
def test_runtime_dispatch_driver_backend_combine_payload_transfer_completion_scaffold_status_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "private completion scaffold/status only",
        "not payload transfer or completion execution",
        ...
        "valid prepared completion scaffold/status remains unsupported",
        "actual_fused_cross_gpu_execution",
        "PtoCudaRuntimeFusionFailure",
        "1U << 31U",
        "no `1U << 32U`",
        ...
        "future private combine payload transfer completion handoff boundary",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Payload Transfer Completion Scaffold Status "
        "Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
        ...
        "uv-xiao/pto-cu",
        "base branch `main`",
        "starting commit `47136ab0b1cff42b7ad3448809a3b9e5bf44db43`",
        "gh pr create --repo uv-xiao/pto-cu --base main --head",
        "src/cuda/platform/include/host/pto_cuda_runtime_fusion_abi.h",
        "tests/ut/py/test_cuda_runtime_fusion_private_entry.py",
        "tests/ut/py/test_nvidia_review_artifacts.py",
...
def test_runtime_dispatch_driver_backend_combine_payload_transfer_completion_handoff_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Payload Transfer Completion Handoff Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_dispatch = " ".join(dispatch_entry.split())
    for required in [
...
def test_runtime_dispatch_driver_backend_combine_payload_transfer_completion_handoff_result_scaffold_status_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    required_terms = [
        ...
        "f425090f",
        "private completion handoff result scaffold/status only",
        "not a real result transport/backend implementation",
        "backend request scaffold/status input",
        "dispatch request scaffold/status dependency",
        "combine request scaffold/status dependency",
        "combine payload descriptor scaffold/status dependency",
        ...
        "public/provenance sourced state",
        "fabricated pass evidence",
        "valid prepared result scaffold/status remains `unsupported`",
        "actual_fused_cross_gpu_execution == 0",
        "no actual fused execution claim",
        ...
        "no completion implementation",
        "no handoff implementation",
        "no result transport implementation",
        "no transport/backend execution",
        "no scheduler/runtime pass evidence",
        "no fresh H200 fused success",
        "no public API expansion",
    ...
    for required in [
        ...
        "result.status == PTO_CUDA_RUNTIME_FUSION_STATUS_UNSUPPORTED",
        "result.actual_fused_cross_gpu_execution == 0U",
        "PTO_CUDA_RUNTIME_FUSION_FAILURE_DRIVER_BACKEND_COMBINE_PAYLOAD_SCAFFOLD",
    ]:
...
def test_runtime_dispatch_driver_backend_combine_payload_transfer_completion_handoff_result_transport_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    normalized_dispatch = " ".join(texts["dispatch"].split())
    assert (
        "Worker id `019f0094-8b8e-7741-b0f9-65a2dc650406`; "
        "worker name `Carver`."
        in normalized_dispatch
    )
    ...
    required_terms = [
        ...
        "d80ccd23",
        "docs/test mapping only",
        "future private result transport boundary",
        "no real UCCL-EP dispatch/combine work",
        "no descriptor allocation behavior change",
        "no payload transfer implementation",
        "no completion implementation",
        ...
        "combine payload transfer completion scaffold/status dependency",
        "combine payload transfer completion handoff scaffold/status dependency",
        "completion handoff result scaffold/status dependency",
        ...
        "`PtoCudaRuntimeFusionFailure` is exhausted through `1U << 31U`",
        "no `1U << 32U`",
        "reuse the existing combine-payload aggregate failure bit",
...
def test_runtime_dispatch_driver_backend_combine_payload_transfer_completion_handoff_result_transport_completion_map_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    normalized_dispatch = " ".join(texts["dispatch"].split())
    assert (
        "Worker id `019f00b4-ac6a-7e60-84f7-8f4662fdc9bc`; "
        "worker name `Confucius`."
        in normalized_dispatch
    )
    ...
    required_terms = [
        ...
        "no handoff implementation",
        "no result transport implementation",
        "no transport completion implementation",
        "no transport/backend execution",
        "no scheduler/runtime pass evidence",
        "no fresh H200 fused success",
        "no public API expansion",
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Payload Transfer Completion Handoff Result "
        "Transport Completion Map Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_entry = " ".join(dispatch_entry.split())
    for required in [
...
def test_runtime_dispatch_driver_backend_combine_payload_transfer_completion_handoff_result_transport_completion_scaffold_status_slice_is_review_safe():
    in_progress_root = ROOT / "docs" / "in_progress" / "nvidia_backend"
    docs = {
        "persistent_moe": in_progress_root
        / "persistent_moe_dispatch_combine_h200.md",
    ...
    normalized_dispatch = " ".join(texts["dispatch"].split())
    assert (
        "Worker id `019f00c2-489c-7a41-9a4a-49080d65d1b2`; "
        "worker name `Boyle`."
        in normalized_dispatch
    )
    ...
    required_terms = [
        ...
        "d16124f0",
        "private result transport completion scaffold/status only",
        "not transport completion implementation",
        "no real UCCL-EP dispatch/combine work",
        "no descriptor allocation behavior change",
        "no payload transfer implementation",
        "no completion implementation",
        ...
        "public/provenance sourced state",
        "fabricated pass evidence",
        "valid prepared result transport completion scaffold/status remains `unsupported`",
        "`actual_fused_cross_gpu_execution == 0`",
        "`PtoCudaRuntimeFusionFailure` is exhausted through `1U << 31U`",
        "no `1U << 32U`",
        "reuses `PTO_CUDA_RUNTIME_FUSION_FAILURE_DRIVER_BACKEND_COMBINE_PAYLOAD_SCAFFOLD`",
        ...
        "selected exactly one next PR-sized docs/test map slice",
        "future private result transport status boundary",
    ]
    for name, text in texts.items():
        normalized = " ".join(text.split())
        for required in required_terms:
    ...
    dispatch_entry = texts["dispatch"].split(
        "### 2026-06-26 - UCCL-EP Runtime Fusion Runtime Dispatch "
        "Driver Backend Combine Payload Transfer Completion Handoff Result "
        "Transport Completion Scaffold Status Worker",
        1,
    )[1].split("\n### ", 1)[0]
    normalized_entry = " ".join(dispatch_entry.split())
    for required in [
