...
# -----------------------------------------------------------------------------
"""Run paired A100/H200 CUDA graph-replay vector and tensor sweeps."""
...
GRAPH_REPLAY_BASELINES: tuple[str, ...] = (
    "direct_driver_graph",
    "direct_driver_graph_sgemm",
)
TENSOR_BASELINES = frozenset({"direct_driver_graph_sgemm"})
SOURCE_PAPERS = (
    {
        "id": "arXiv:2605.03190",
        "label": "VDCores",
...
PAPER_SETUP = (
    "CUDA graph-replay sweep for selected vector and tensor launch shapes; "
    "not an end-to-end LLM serving result."
)


@dataclass(frozen=True)
class GraphReplaySweepConfig:
...
def build_remote_sync_command(config: GraphReplaySweepConfig) -> list[str]:
    return [
        "rsync",
        "-a",
        "--delete",
...
def _remote_prefix(config: GraphReplaySweepConfig) -> list[str]:
...
def _tensor_tile(config: GraphReplaySweepConfig) -> dict[str, int]:
    return {
...
def _validate_baselines(baselines: Sequence[str]) -> tuple[str, ...]:
    ...
    if not normalized:
        raise ValueError("at least one graph replay baseline is required")
    unknown = [
        baseline for baseline in normalized if baseline not in GRAPH_REPLAY_BASELINES
    ]
    if unknown:
        allowed = ", ".join(GRAPH_REPLAY_BASELINES)
        raise ValueError(f"unknown graph replay baseline(s): {unknown}; allowed: {allowed}")
    return normalized
...
def output_dir_for(config: GraphReplaySweepConfig, commit: str) -> Path:
    return config.output_root / f"graph-replay-sweep-{commit}"
...
def run_graph_replay_sweep(
    ...
    *,
    ...
    payload = {
        "metadata": {
            "label": f"graph-replay-sweep-{commit}",
            "git_commit": commit,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    run_graph_replay_sweep(config, dry_run=args.dry_run)
    if not args.dry_run:
