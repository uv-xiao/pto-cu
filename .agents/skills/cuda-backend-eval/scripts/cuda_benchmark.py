...
from cuda_benchmark_impl.fallback_ptx import (
    SLOW_VECTOR_ADD_PTX as _FALLBACK_SLOW_VECTOR_ADD_PTX,
)
from cuda_benchmark_impl.contracts import (
...
def run_single_sample(  # noqa: PLR0912
    ...
    if baseline == "pto_persistent_dag_graph_node_link":
        return run_persistent_sample(
            device=device,
            n=n,
            arch=arch,
    ...
    if baseline == "pto_persistent_dag_graph_scalar_scale":
        return run_persistent_sample(
            device=device,
            n=n,
            arch=arch,
    ...
    if baseline == "pto_persistent_dag_graph_wide_fanout":
        return run_persistent_sample(
            device=device,
            n=n,
            arch=arch,
    ...
    if baseline == "pto_persistent_dag_graph_pair_inout":
        return run_persistent_sample(
            device=device,
            n=n,
            arch=arch,
...
def render_svg(summary: dict[tuple[str, str, int, int, int], dict[str, Any]]) -> str:
    ...
    colors = {
        ...
        "pto_persistent_dag_graph_depends_on": "#4f6f52",
    ...
    if graph_metadata:
        lines.extend(
            [
                ...
                "Graph task arg key | Graph task args | Graph node attrs | Graph node ops | "
