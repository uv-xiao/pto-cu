...
plus bridges to the repeated proxy microdecode runner and repeated unit-math
...
PYTHONPATH=$PWD:$PWD/python \
  .venv/bin/python examples/cuda/qwen_decode_loop_runner.py \
    ...
    --repeat-runs 3 \
    --output-json tmp/cuda-backend/pto-serving-decode-loop-token-kv-resident-live-$(git rev-parse --short HEAD)/qwen-decode-loop-runner.json
```
