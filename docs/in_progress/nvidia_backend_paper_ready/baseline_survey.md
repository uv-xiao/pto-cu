...
also has a safetensors metadata probe that reports the five real Qwen shards
missing locally. It still does not open the real Qwen safetensors tensors,
validate actual safetensors metadata, bind Qwen weights to CUDA buffers, bind
token IDs to runtime buffers, bind real CUDA buffers, run generated Qwen kernel
bodies, or execute a decode loop for `Qwen/Qwen3-8B`.
