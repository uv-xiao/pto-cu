# vLLM Remote H200 Long-Prompt Response-Contract Probe
...
extends the prior 16K-ish long-prompt admission gate by validating
review-safe OpenAI-compatible response structure for the same request class,
without judging generated text.

Raw command output is kept under the gitignored local directory
`tmp/vllm-long-prompt-response-contract-probe/`.
...
non-streaming `/v1/completions` request near a 16K prompt-token budget,
...
The remote source tree was refreshed with `--sync` before the preflight. The
...
```text
127.0.0.1:28136 available: yes
```

The planned local-only endpoints were:

```text
GET http://127.0.0.1:28136/health
GET http://127.0.0.1:28136/v1/models
POST http://127.0.0.1:28136/v1/completions
```
...
REMOTE_PTO_CU=/tmp/pto-cu-vllm-remote-env-artifact \
  ...
  bash -lc 'RUN_DIR=tmp/vllm-long-prompt-response-contract-probe
RUN_LOG="$RUN_DIR/server-28136.log"
mkdir -p "$RUN_DIR"
...
Starting vLLM server on http://127.0.0.1:28136
...
The server log also recorded first-request Triton JIT warnings during the
long-prompt completion. Those warnings are response-contract observations
only, not latency or throughput evidence.
...
The remote H200 vLLM environment served one bounded 16K-ish prompt completion
...
Run a bounded long-prompt warmup-shape probe for the same 16K-ish request
class to separate first-request JIT observations from repeated same-shape
serving behavior without adding generated-text correctness claims.
