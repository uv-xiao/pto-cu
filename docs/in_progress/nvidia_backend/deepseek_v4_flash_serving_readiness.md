...
chat-completions synthetic needle exact-repeat pass gate, plus one bounded
OpenAI-compatible near-256K streaming chat-completions synthetic needle
truncated-generation failure-mode gate:
...
serving_readiness: bounded local-only response contract and warmup-shape
  ...
  gate, plus one bounded OpenAI-compatible near-256K streaming
  chat-completions synthetic needle truncated-generation failure-mode gate;
  no general correctness claims
```
...
`/v1/chat/completions` synthetic needle exact-output requests under one
recorded 262144-token vLLM server lifecycle. The requests targeted a
255800-token prompt budget, used `max_tokens=64`, `temperature=0.0`,
`top_p=1.0`, `seed=0`, expected answer
`PTO_CHAT_NEEDLE_256K_REPEAT_OK_28152`, strict exact match mode, and stop
sequence `"\n```"`. Both attempts returned HTTP 200 with one response choice,
`finish_reason=stop`, usage reported `prompt_tokens=255796`,
`completion_tokens=19`, and `total_tokens=255815`, and the narrowly
normalized assistant content exactly matched the expected answer. The repeat
aggregate recorded only review-safe request limits, response shape, usage,
exact-match status, and cleanup state; it did not record raw prompt text, raw
request payload, raw generated text, token ID arrays, logprob values,
generated-text digests, model artifact contents, non-loopback URLs, hostnames,
usernames, or private absolute paths.

Detailed chat-completions near-256K needle exact-repeat evidence is recorded
in
...
the same recorded 262144-token vLLM server boundary. The request targeted a
255800-token prompt budget, used `max_tokens=1`, `temperature=0.0`,
`top_p=1.0`, `seed=0`, expected answer
`PTO_CHAT_NEEDLE_256K_STREAM_TRUNCATED_OK_28155`, strict exact match mode,
stop sequence `"\n```"`, and `stream=true`. The streaming path returned HTTP
200, parsed SSE chunk events, received terminal `[DONE]`, reported
`finish_reason=length`, did not return streaming usage, and strict exact mode
failed with `chat_needle_stream_expected_answer_not_exact`. The probe
recorded only review-safe request limits, streaming event counters, usage
state, exact-match status, and cleanup state; it did not record raw prompt
text, raw request payload, raw generated text, raw streaming chunk content,
token ID arrays, logprob values, generated-text digests, model artifact
contents, non-loopback URLs, hostnames, usernames, or private absolute paths.
This is an expected strict exact-comparator failure-mode characterization,
not a transport/server failure and not generated-text correctness evidence.

Detailed chat-completions near-256K needle streaming truncated-generation
...
It has now also recorded one local-only OpenAI-compatible near-256K streaming
`/v1/chat/completions` synthetic needle usage-contract attempt under the same
...
The request targeted a 255800-token prompt budget, used `max_tokens=64`,
`temperature=0.0`, `top_p=1.0`, `seed=0`, expected answer
`PTO_CHAT_NEEDLE_256K_STREAM_USAGE_OK_28157`, strict exact match mode, stop
sequence `"\n```"`, `stream=true`, and
...
and saw streaming usage keys on a final usage-bearing event. The probe exited
with `PROBE_EXIT_STATUS=2` and failed with
`chat_needle_stream_choice_shape` because that final usage-bearing event had
zero choices, while the current streaming parser requires exactly one choice
for every JSON event. Terminal `[DONE]`, final `finish_reason`, strict exact
output matching, and usage-accounting pass/fail checks are not established by
this run. The probe recorded only review-safe request limits, streaming event
counters, usage-key presence, exact-match non-status, failure category, and
cleanup state; it did not record raw prompt text, raw request payload, raw
generated text, raw streaming chunk content, token ID arrays, logprob values,
generated-text digests, model artifact contents, non-loopback URLs,
hostnames, usernames, or private absolute paths. This is a completed
usage-contract failure-mode characterization, not generated-text correctness
evidence and not a usage-accounting pass.
...
The next PR-sized gate is to decide whether the streaming parser should
accept OpenAI-style final usage-only chunks with zero choices, then rerun the
same usage-contract command if that contract is updated. Do not claim a
usage-accounting pass or strict exact-output pass for port `28157` until a
future structured result records those checks as passed.
