# vLLM Remote H200 64K Long-Prompt Response-Contract Probe
...
extends the prior 32K long-prompt response-contract gate by raising the
synthetic prompt budget to 64K prompt tokens while preserving the same
review-safe response/accounting contract.

Raw command output is kept under the gitignored repo-relative directory
`tmp/vllm-64k-long-prompt-response-contract-probe/`.
...
non-streaming `/v1/completions` request near a 64K prompt-token budget,
...
```text
GET http://127.0.0.1:28139/health
GET http://127.0.0.1:28139/v1/models
POST http://127.0.0.1:28139/v1/completions
```
...
usage.prompt_tokens: 63999
usage.completion_tokens: 4
usage.total_tokens: 64003
...
baseline for GPU 1 and GPU 7.

## Interpretation

The remote H200 vLLM environment served one bounded 64K prompt completion
...
64K long-prompt response-contract gate only.
