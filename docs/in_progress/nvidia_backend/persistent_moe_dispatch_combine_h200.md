...
scaffold/status only, not result transport/backend execution. No fresh H200
fused run is recorded for this scaffold/status slice.
...
only for the future private result transport completion boundary. No fresh
H200 fused run is recorded for this map slice.
...
This H200 status note records the
`nvidia-uccl-ep-runtime-fusion-runtime-dispatch-driver-backend-combine-payload-transfer-completion-handoff-result-transport-completion-scaffold-status`
