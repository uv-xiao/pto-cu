# vLLM Remote H200 Chat 256K Needle Streaming Usage Contract Probe

This note records bounded OpenAI-compatible streaming chat-completions
synthetic needle usage-contract attempts for
`deepseek-ai/DeepSeek-V4-Flash`. The latest run uses the running vLLM
server's `/tokenize` endpoint to measure the same chat messages before the
streaming request, then compares returned streaming `usage.prompt_tokens`
against that server-side count.

Earlier attempts are still useful context: the first run found that vLLM can
return usage in a final usage-bearing event with zero choices, and the parser
contract was then updated to accept that OpenAI-style final zero-choice usage
chunk. The next run passed exact output matching but failed the usage
accounting gate because `usage_prompt_tokens_match` was `not_available`.

Raw command output is kept under gitignored repo-relative `tmp/` directories.
The latest run output is under
`tmp/vllm-chat-256k-needle-stream-usage-token-accounting/`.

## Probe Surface

The repo-owned usage-contract entry point reuses the chat 256K streaming
needle prompt builder, lifecycle, readiness contract, and exact normalization,
then adds the OpenAI streaming usage option and a server-side tokenizer
measurement:

```text
tokenize_endpoint: /tokenize
chat_endpoint: /v1/chat/completions
stream: true
stream_options.include_usage=true
expected_answer: PTO_CHAT_NEEDLE_256K_STREAM_USAGE_OK_28159
stop: ["\n```"]
match_mode: exact
```

The contract requires returned streaming usage, strict exact output matching,
and a prompt-token accounting match between `usage.prompt_tokens` and the
server-side `/tokenize` chat count. The probe records only review-safe
endpoint names, status values, request limits, shape keys, and integer token
counts; it does not record raw prompt text, raw payloads, raw generated text,
raw streaming chunk content, token IDs, or logprob values.

## Remote Preflight

The preserved remote vLLM probe environment was reused. The selected remote
checkout was refreshed with `--sync`, preserving the existing remote vLLM
probe venv and repo-relative model artifact directory.

The preflight showed H200 visibility and required prerequisites:

```text
8 x NVIDIA H200 NVL
compute capability: 9.0
driver: 580.126.20
memory.total: 143771 MiB per GPU
python_status=0
vllm_status=0
artifact_status=0
```

## Server-Tokenize Usage-Contract Rerun

Core probe command shape:

```bash
.agents/skills/cuda-backend-eval/scripts/run-remote-cuda.sh \
  --remote-dir <remote-pto-cu> --sync -- \
  bash -lc '<bounded command that preserves PROBE_EXIT_STATUS>'

CUDA_VISIBLE_DEVICES=1,7 VLLM_NO_USAGE_STATS=1 \
PYTHONPATH=$PWD:$PWD/python \
timeout --foreground 80m \
.venv-vllm-probe/bin/python \
examples/cuda/vllm_deepseek_v4_chat_256k_needle_stream_usage_contract_probe.py \
  --artifact-dir tmp/model-artifacts/deepseek-ai/DeepSeek-V4-Flash \
  --vllm-bin .venv-vllm-probe/bin/vllm \
  --port 28159 \
  --server-log tmp/vllm-chat-256k-needle-stream-usage-token-accounting/server-28159.log \
  --max-model-len 262144 --tensor-parallel-size 2 \
  --dtype bfloat16 --quantization deepseek_v4_fp8 \
  --kv-cache-dtype fp8 --gpu-memory-utilization 0.78 \
  --distributed-executor-backend mp --enforce-eager \
  --timeout-seconds 2700 --poll-interval-seconds 10 \
  --request-timeout-seconds 180 --terminate-timeout-seconds 60 \
  --target-prompt-tokens 255800 --max-tokens 64 \
  --temperature 0.0 --top-p 1.0 --seed 0 \
  --expected-answer PTO_CHAT_NEEDLE_256K_STREAM_USAGE_OK_28159
```

Result:

```text
status: passed
server_host: 127.0.0.1
server_port: 28159
PROBE_EXIT_STATUS=0
generation_attempted: true
prompt_sent: true
stream_events_received: true
elapsed_seconds: 153.989
```

Readiness and request controls:

```text
/health HTTP status: 200
/v1/models HTTP status: 200
model_list includes: deepseek-ai/DeepSeek-V4-Flash
model_list max_model_len: 262144
endpoint: /v1/chat/completions
tokenize_endpoint: /tokenize
max_model_len=262144
tensor_parallel_size=2
dtype=bfloat16
quantization=deepseek_v4_fp8
kv_cache_dtype=fp8
gpu_memory_utilization=0.78
distributed_executor_backend=mp
enforce_eager=true
target_prompt_tokens=255800
actual_prompt_tokens: 255797
prompt_token_measurement_source: vllm_server_tokenize_chat_count
prompt_token_measurement_status: passed
prompt_token_measurement_http_status: 200
tokenizer_accounting: vLLM server /tokenize chat count
prompt_chars: 1233756
prompt_unit_chars: 82
max_tokens=64
temperature=0.0
top_p=1.0
seed=0
expected_answer: PTO_CHAT_NEEDLE_256K_STREAM_USAGE_OK_28159
needle_occurrences: 1
needle_position: middle
message_count: 2
message_roles: system,user
match_mode: exact
stop_sequences_configured: true
stop: ["\n```"]
stream: true
stream_options.include_usage=true
n: 1
prompt_text_recording: false
payload_recording: false
assistant_content_recording: false
```

Streaming endpoint result:

```text
HTTP status: 200
stream_events_received: true
event_count: 22
content_chunk_count: 18
done_seen: true
finish_reason: stop
normalized_output_equals_expected: true
expected_answer_exact: passed
usage_presence: passed
usage_shape: passed
usage_prompt_tokens_match: passed
usage_completion_bound: passed
usage_total_tokens: passed
usage.prompt_tokens: 255797
usage.completion_tokens: 20
usage.total_tokens: 255817
```

Review-safe response shape observations:

```text
ordinary streaming content events:
  choice_count: 1
  first_choice_review_safe_keys: delta,finish_reason,index
  delta_review_safe_keys: content or content,role
  top_level_review_safe_keys: choices,created,id,model,object
  usage_keys: []
final usage-bearing event:
  choice_count: 0
  top_level_review_safe_keys: choices,created,id,model,object,system_fingerprint,usage
  usage_keys: completion_tokens,prompt_tokens,total_tokens
```

Cleanup:

```text
cleanup.status: passed
terminated: true
killed: false
returncode_after_cleanup: 0
remaining_process_group_pids: []
```

## Interpretation

This is a local-only two-H200 streaming `/v1/chat/completions` synthetic
needle usage-contract pass under the recorded 262144-token vLLM server
boundary. The server reached readiness, `/tokenize` returned HTTP 200 for the
same chat messages, the streaming request returned HTTP 200, content SSE
events were parsed, terminal `[DONE]` was received, and vLLM returned a
streaming usage object with prompt, completion, and total token keys in a
final zero-choice usage event.

The latest run passed the usage-accounting gate:
`usage.prompt_tokens=255797` matched the server-side `/tokenize` chat count
of `255797`. It also passed the completion bound, total-token bound, final
zero-choice usage chunk handling, cleanup, and strict exact output match.

This is review-useful usage-contract evidence, not a broad generated-text or
production-readiness claim.

## Review-Safety Omissions

```text
raw prompt text is not recorded
raw request payload is not recorded
raw generated text is not recorded
raw streaming chunk content is not recorded
token ID arrays are not recorded
logprob values are not recorded
generated-text digests are not recorded
model artifact contents and symlinks are not recorded
private absolute paths are not recorded
hostnames are not recorded
usernames are not recorded
non-loopback URLs are not recorded
```

## Non-Claims

- This is not general generated-text correctness evidence.
- This is not semantic correctness evidence.
- This is not tokenizer semantic correctness evidence.
- This is not token identity evidence.
- This is not logprob correctness evidence.
- This is not broad stop-trigger or stop-token semantic correctness evidence.
- This is not throughput or latency evidence.
- This is not production-readiness evidence.
- This is not broad determinism evidence.
- This is not simpler-nv or vLLM integration evidence.
