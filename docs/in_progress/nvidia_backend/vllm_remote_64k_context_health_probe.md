# vLLM Remote H200 64K Context Health Probe

This note records a bounded remote H200 vLLM server-health/model-list probe
for `deepseek-ai/DeepSeek-V4-Flash` with `--max-model-len 65536`. It reuses
...
`tmp/vllm-64k-context-health-probe/`.
...
model-list max_model_len is 65536
...
```text
127.0.0.1:28132 available: yes
```

The planned local-only endpoints were:

```text
GET http://127.0.0.1:28132/health
GET http://127.0.0.1:28132/v1/models
```
...
REMOTE_PTO_CU=/tmp/pto-cu-vllm-remote-env-artifact \
  ...
  bash -lc 'RUN_DIR=tmp/vllm-64k-context-health-probe
RUN_LOG="$RUN_DIR/server-28132.log"
mkdir -p "$RUN_DIR"
...
Loading weights took 32.64 seconds
Model loading took 74.08 GiB memory and 38.499518 seconds
Available KV cache memory: 32.08 GiB
GPU KV cache size: 673,323 tokens
Maximum concurrency for 65,536 tokens per request: 10.27x
init engine (profile, create kv cache, warmup model) took 10.21 s
Starting vLLM server on http://127.0.0.1:28132
...
The final server log emitted a Python `resource_tracker` shared-memory cleanup
warning at shutdown. The probe still reported no remaining process-group PIDs,
and selected GPU memory returned to the baseline above.
...
the served model with `max_model_len=65536` from `/v1/models`, and shut down
with no remaining process-group PIDs reported by the probe. This is a 64K
...
The next reviewable context-capacity gate can attempt the next smaller
unproven serving boundary between 64K and the final 256K target, for example
128K server-health/model-list readiness under the same local-only contract
without sending a long prompt. That later gate should keep generation and
correctness claims out of scope unless a separate bounded request contract is
reviewed.
