# vLLM Remote H200 192K Long-Prompt Response-Contract Probe
...
extends the prior 128K long-prompt response-contract gate by raising the
synthetic prompt budget to 192K prompt tokens while preserving the same
review-safe response/accounting contract.

Raw command output is kept under the gitignored repo-relative directory
`tmp/vllm-192k-long-prompt-response-contract-probe/`.
...
non-streaming `/v1/completions` request near a 192K prompt-token budget,
...
source commit `95503d8004472f3be446155b59550ca9b287701e`, preserving the
...
```text
GET http://127.0.0.1:28141/health
GET http://127.0.0.1:28141/v1/models
POST http://127.0.0.1:28141/v1/completions
```

The dry-run preflight emitted `status=planned` for `target_prompt_tokens=192000`
...
```text
target_prompt_tokens: 192000
actual_prompt_tokens: 191995
prompt_chars: 1256669
max_tokens: 4
temperature: 0.0
top_p: 1.0
seed: 0
...
usage.prompt_tokens: 191995
usage.completion_tokens: 4
usage.total_tokens: 191999
...
port 28141 free after cleanup.

## Interpretation

The remote H200 vLLM environment served one bounded 192K prompt completion
...
192K long-prompt response-contract gate only.
