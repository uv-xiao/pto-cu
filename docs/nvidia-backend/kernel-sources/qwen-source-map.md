...
| QK norm, RoPE, attention | FlashInfer decode/QK-RoPE, SGLang fused QKNorm-RoPE | PTO has shape-field QK RMSNorm/RoPE formula source, descriptor-level RoPE table slots, launch-packet live RoPE pointer binding, Qwen-theta first-position table population, and dynamic per-step RoPE refresh for bounded decode-step runs; decode attention remains open. |
