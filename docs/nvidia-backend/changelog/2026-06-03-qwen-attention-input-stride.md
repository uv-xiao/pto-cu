# 2026-06-03 Qwen Attention Input Stride

## Code And Data Changed

- Updated `qwen_attention_o` generated CUDA source to use `a_batch_stride` for
  the batch-row input base instead of overloading `lda`.
- Updated the Qwen attention-output shape contract so descriptors carry the
  QK-norm output row stride, `q_width + kv_width`.
...
This separates two meanings that were previously conflated in the
attention-output task body. `lda` remains the head dimension used for GQA head
mapping and attention scaling, while `a_batch_stride` now describes the
producer layout from QK-norm. This prevents batch rows after row zero from
reading query values out of the wrong row slice.
...
- Passed live A100 MPK-policy diagnostic:
  ...
  ```bash
  ARTIFACT=tmp/cuda-backend/qwen-attention-input-stride-mpk-2026-06-03
