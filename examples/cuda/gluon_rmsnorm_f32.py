#!/usr/bin/env python3
"""Skip-safe correctness harness for generated Gluon FP32 RMSNorm."""
...
DEFAULT_OUTPUT_DIR = Path("tmp/gluon-rmsnorm-local")
RMSNORM_REFERENCE = (
...
def rmsnorm_skip_reason() -> str | None:
    try:
...
def run_rmsnorm_correctness(
    *,
    ...
    artifact = build_rmsnorm_artifact(output_dir=resolved_output_dir, arch=arch)
    result = {
        "schema_version": 1,
    ...
    skip_check = rmsnorm_skip_reason if skip_reason is None else skip_reason
    reason = skip_check()
    ...
    x = torch.randn((rows, hidden), device=cuda_device, dtype=torch.float32, generator=generator)
    weight = torch.randn((hidden,), device=cuda_device, dtype=torch.float32, generator=generator)
    out = torch.empty((rows, hidden), device=cuda_device, dtype=torch.float32)

    module.run_rmsnorm_f32(x, weight, out, eps=eps)
    torch.cuda.synchronize(device)

    expected = x * torch.rsqrt(x.pow(2).mean(dim=-1, keepdim=True) + eps) * weight
    torch.cuda.synchronize(device)
...
def run_rmsnorm_sweep(
    *,
    ...
    case_specs = RMSNORM_SWEEP_CASES if cases is None else cases
    case_results = []
    ...
    for index, case in enumerate(case_specs):
        ...
        eps = float(case["eps"])
        try:
            result = run_rmsnorm_correctness(
...
def _display_command(raw_args: list[str]) -> str:
    ...
    command = "examples/cuda/gluon_rmsnorm_f32.py"
    if safe_args:
