...
digest metadata, and validation metadata when available. The Min-P sampling
...
to one. The default review shape remains
`rows=2, vocab=8, max_k=5, min_p=0.5`; the broader fixture uses
`rows=3, vocab=16, max_k=6, min_p=0.5` to cover varied selected counts
and ties. The operator selects candidates whose probability is at least
`min_p` times the row maximum, sorts selected candidates by probability
descending with lower token id first for ties, and fills unused output slots
with `0.0` values and `-1` indices. The JSON includes shape, dtype, request
metadata, CPU golden `values`, `indices`, `selected_counts`, GPU result
fields when CUDA runs, validation flags including payload shape checks,
...
The prefill-shaped JSON records `phase: prefill`, `causal: true`,
`seqlen_q=32`, `seqlen_k=32`, `head_dim=64`, tolerance, repo-relative
artifact paths, and the lower-triangular PyTorch reference formula
`softmax(masked_fill((q @ k.T) * scale, key_index > query_index, -inf)) @ v`.
It is same-length multi-query prefill-shaped correctness evidence only, not
FlashInfer integration, vLLM/simpler-nv integration, paged/ragged KV-cache
coverage, full prefill, full decode, full append, serving readiness, or
...
For explicit Cascade Attention unsupported-boundary evidence, use:
...
This command emits structured skip JSON before CUDA availability checks. The
JSON records `schema_version`, `status: skipped`, `phase: prefill`,
`causal: true`, `attention_variant: cascade`, shape metadata, tolerance, and
`unsupported_boundary` with stable `kind: cascade_attention`. This is
unsupported-boundary evidence only, not Cascade Attention correctness, MLA
attention correctness, varlen attention correctness, paged/ragged KV-cache
...
For explicit POD-Attention unsupported-boundary evidence, use:
...
This command emits structured skip JSON before CUDA availability checks. The
JSON records `schema_version`, `status: skipped`, `phase: prefill`,
`causal: true`, `attention_variant: pod`, shape metadata, tolerance, and
`unsupported_boundary` with stable `kind: pod_attention`. This is
unsupported-boundary evidence only, not POD-Attention correctness, Sparse
Attention correctness, Cascade Attention correctness, MLA attention
correctness, varlen attention correctness, paged/ragged KV-cache correctness,
full prefill/decode/append coverage, serving integration, or performance
evidence.
