...
consumption evidence, and a controlled proxy numeric oracle.
...
PYTHONPATH=$PWD:$PWD/python \
  ...
  --device 0 \
  --arch compute_80 \
  --output-json tmp/cuda-backend/pto-serving-microdecode-live/qwen-microdecode-live.json
```

Expected output: command exits 0 on a CUDA host; output JSON records
status=pass for a controlled Qwen QKV-to-logits proxy DAG launched through
cuda/persistent_device.
...
KV writeback plus final logits copy-back, and still remains controlled proxy
