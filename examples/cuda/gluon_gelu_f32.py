#!/usr/bin/env python3
"""Skip-safe correctness harness for generated Gluon FP32 GELU."""
...
DEFAULT_OUTPUT_DIR = Path("tmp/gluon-gelu-local")
GELU_REFERENCE = "0.5 * x * (1.0 + erf(x / sqrt(2.0)))"
GELU_SWEEP_CASES = [
    {
        "name": "existing_smoke",
        "n": 32,
...
def gelu_skip_reason() -> str | None:
    ...
    if not hasattr(gl, "erf"):
        return "triton Gluon import failed: missing gl.erf"
...
def run_gelu_correctness(
    *,
    ...
    artifact = build_gelu_artifact(output_dir=resolved_output_dir, arch=arch)
    result = {
        "schema_version": 1,
        "kernel_name": "gelu_f32",
    ...
    skip_check = gelu_skip_reason if skip_reason is None else skip_reason
    reason = skip_check()
    ...
    x = torch.linspace(-4.0, 4.0, n, device=cuda_device, dtype=torch.float32)
    out = torch.empty((n,), device=cuda_device, dtype=torch.float32)

    module.run_gelu_f32(x, out)
...
def run_gelu_sweep(
    *,
    ...
    case_specs = GELU_SWEEP_CASES if cases is None else cases
    case_results = []
...
def _display_command(raw_args: list[str]) -> str:
    ...
    command = "examples/cuda/gluon_gelu_f32.py"
    if safe_args:
