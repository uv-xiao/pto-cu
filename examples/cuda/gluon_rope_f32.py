#!/usr/bin/env python3
"""Skip-safe correctness harness for generated Gluon FP32 RoPE."""
...
def run_rope_correctness(
    *,
    ...
    if batch <= 0 or seq <= 0 or head_dim <= 0:
        raise ValueError("--batch, --seq, and --head-dim must be positive")
    if head_dim % 2 != 0:
        raise ValueError("--head-dim must be even")

    artifact = build_rope_artifact(output_dir=resolved_output_dir, arch=arch)
    result = {
        "schema_version": 1,
    ...
    skip_check = rope_skip_reason if skip_reason is None else skip_reason
    reason = skip_check()
    ...
    trig = torch.arange(seq * (head_dim // 2), device=cuda_device, dtype=torch.float32)
    trig = trig.reshape(seq, head_dim // 2) * 0.125
    cos = torch.cos(trig)
    sin = torch.sin(trig)
    out = torch.empty((batch, seq, head_dim), device=cuda_device, dtype=torch.float32)

    module.run_rope_f32(x, cos, sin, out)
    torch.cuda.synchronize(device)

    expected = torch.empty_like(x)
    x_even = x[..., 0::2]
    x_odd = x[..., 1::2]
    expected[..., 0::2] = x_even * cos - x_odd * sin
    expected[..., 1::2] = x_even * sin + x_odd * cos
    torch.cuda.synchronize(device)
