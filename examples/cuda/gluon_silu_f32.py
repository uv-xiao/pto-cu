#!/usr/bin/env python3
"""Skip-safe correctness harness for generated Gluon FP32 SiLU."""
...
DEFAULT_OUTPUT_DIR = Path("tmp/gluon-silu-local")
SILU_REFERENCE = "out = x / (1.0 + exp(-x))"
SILU_SWEEP_CASES = [
    {
        "name": "existing_smoke",
        "n": 32,
...
def run_silu_correctness(
    *,
    ...
    artifact = build_silu_artifact(output_dir=resolved_output_dir, arch=arch)
    result = {
        "schema_version": 1,
        "kernel_name": "silu_f32",
    ...
    skip_check = silu_skip_reason if skip_reason is None else skip_reason
    reason = skip_check()
...
def run_silu_sweep(
    *,
    ...
    case_specs = SILU_SWEEP_CASES if cases is None else cases
    case_results = []
...
def _display_command(raw_args: list[str]) -> str:
    ...
    command = "examples/cuda/gluon_silu_f32.py"
    if safe_args:
