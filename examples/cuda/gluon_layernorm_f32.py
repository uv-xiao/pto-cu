#!/usr/bin/env python3
"""Skip-safe correctness harness for generated Gluon FP32 LayerNorm."""
...
def layernorm_skip_reason() -> str | None:
    try:
...
def run_layernorm_correctness(
    *,
    ...
    artifact = build_layernorm_artifact(output_dir=resolved_output_dir, arch=arch)
    result = {
        "schema_version": 1,
    ...
    skip_check = layernorm_skip_reason if skip_reason is None else skip_reason
    reason = skip_check()
    ...
    bias = torch.randn((hidden,), device=cuda_device, dtype=torch.float32, generator=generator)
    out = torch.empty((rows, hidden), device=cuda_device, dtype=torch.float32)

    module.run_layernorm_f32(x, weight, bias, out, eps=eps)
    torch.cuda.synchronize(device)

    mean = x.mean(dim=-1, keepdim=True)
    var = (x - mean).pow(2).mean(dim=-1, keepdim=True)
    expected = (x - mean) * torch.rsqrt(var + eps) * weight + bias
    torch.cuda.synchronize(device)
...
def _display_command(raw_args: list[str]) -> str:
    ...
    command = "examples/cuda/gluon_layernorm_f32.py"
    if safe_args:
