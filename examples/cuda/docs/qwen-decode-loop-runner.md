...
external-scale bridge. This mode is slower and still diagnostic, but each
input, post-attention, and final RMSNorm task reduces one full hidden vector.
Use `--resource-backed-numeric-task-mode model_equivalent` for Qwen attempts
that combine full RMSNorm, full QKV projection, and full logits windows.
